...
<template>
    <h1 class="font-weight-medium mb-2"><?= $title; ?>
        <v-btn color="success" class="float-end" href="<?= base_url('excel/import') ?>" title="Import Data" alt="Import Data" elevation="1">
            <v-icon>mdi-file-excel-box</v-icon> Import
        </v-btn>
    </h1>
    <v-card>
        <v-card-title>
            <v-btn color="primary" large dark class="mr-2" href="<?= base_url('barang/baru') ?>" title="<?= lang('App.add') . ' ' . $title; ?>" alt="<?= lang('App.add') . ' ' . $title; ?>" elevation="1">
                <v-icon>mdi-plus</v-icon> <?= lang('App.add'); ?>
            </v-btn>
            <v-spacer></v-spacer>
            <v-btn small outlined color="success" class="py-4 mr-1" @click="excelMultiple(selected)" :disabled="selected == ''" :loading="loading4" title="Export Data" alt="Export Data" elevation="1">
                <v-icon>mdi-download</v-icon> Export <span v-show="selected != ''">({{this.selected.length}})
            </v-btn>
            <v-btn small outlined @click="barcodeMultiple(selected)" :disabled="selected == ''" :loading="loading6" class="py-4 mr-1" title="Print Barcode" alt="Print Barcode" elevation="1">
                <v-icon>mdi-barcode</v-icon> Barcode <span v-show="selected != ''">({{this.selected.length}})</span>
            </v-btn>
            <v-btn small outlined @click="labelMultiple(selected)" :disabled="selected == ''" :loading="loading5" class="py-4 mr-1" title="Print Label Rack" alt="Print Label Rack" elevation="1">
        ...
        <!-- Start Table -->
        <v-data-table v-model="selected" item-key="id_barang" show-select :headers="dataTable" :items="dataBarang" :items-per-page="-1" hide-default-footer :loading="loading" :search="search" loading-text="<?= lang('App.loadingWait'); ?>" dense>
            <template v-slot:top>
