...
class ShiftOpenClose extends BaseControllerApi
{
    ...
    {
        $checkIsOpen = $this->model->where(['type' => 'open', 'id_login' => session()->get('id'), 'tanggal' => date('Y-m-d')])->first();
        $checkIsClose = $this->model->where(['type' => 'close', 'id_login' => session()->get('id'), 'tanggal' => date('Y-m-d')])->first();
        $data = "";
        if (!empty($checkIsOpen) && empty($checkIsClose)) {
            $data = 'Open';
        } else if (!empty($checkIsOpen) && !empty($checkIsClose)) {
            $data = 'Close';
        } else {
            $data = 'Absen';
        }
    ...
    public function create()
    {
        ...
        $checkIsOpen = $this->model->where(['type' => 'open', 'id_login' => session()->get('id'), 'tanggal' => $tanggal])->first();
        $checkIsClose = $this->model->where(['type' => 'close', 'id_login' => session()->get('id'), 'tanggal' => $tanggal])->first();
        ...
        if (!$this->validate($rules)) {
            ...
            // Periksa jika type Close belum Open Cashier
                ...
                return $this->respond($response, 200);
            /* } else if ($type == 'close' && date('H:i:s') < strtotime($waktu)) {
                $response = [
                    'status' => false,
                    'message' => 'Belum Jam Akhir Shift',
                    'data' => [],
                ];
                return $this->respond($response, 200); */
            } else {
