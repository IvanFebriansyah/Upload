...
<template>
    ...
    <?php if (session()->getFlashdata('error')) { ?>
        <v-alert colored-border type="error" prominent dense border="right" elevation="2" icon="mdi-alert-octagon" class="font-weight-medium" v-model="alert">
    ...
    <v-card>
        ...
        <v-data-table v-model="selected" item-key="id_shift_openclose" show-select :headers="dataTable" :items="data" :options.sync="options" :server-items-length="totalData" :items-per-page="10" :loading="loading" :search="search" loading-text="<?= lang('App.loadingWait'); ?>">
            <template v-slot:top>
            ...
            axios.get(`<?= base_url() ?>api/shift/${this.idShift}`, options)
                .then(res => {
                    ...
                    if (data.status == true) {
                        ...
                        this.waktu = this.shift.jam_selesai;
                    } else {
                        this.snackbar = true;
                        this.snackbarMessage = data.message;
                        this.shift = data.data;
